      ...
      {conversations.map((conv) => (
        ...
        >
          <div className="relative flex-shrink-0">
            ...
            />
            {conv.isOnline && (
              <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-[var(--bg-secondary)] rounded-full"></div>
            )}
          </div>
          
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-baseline mb-0.5">
              <h4 className="font-medium text-[var(--text-primary)] truncate text-sm">{conv.name}</h4>
