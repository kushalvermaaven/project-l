...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(role === 'artist' ? '/artist-dashboard' : '/buyer-dashboard');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex bg-[#0a0a0f] text-[#f0f0f5]">
      ...
      <div className="hidden lg:flex flex-1 flex-col justify-center items-center relative overflow-hidden bg-gradient-to-br from-[#13131a] to-[#1a1a2e] border-r border-white/10 p-12">
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-cyan-900/20 via-[#0a0a0f]/0 to-purple-900/20"></div>
          ...
          {step === 1 ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center mb-10">
                <h2 className="text-3xl font-heading font-bold text-[#f0f0f5] mb-2">Join Artvrkz</h2>
                <p className="text-[#a0a0b8]">What are you joining as?</p>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-6 mb-8">
                ...
                >
                  ...
                  <p className="text-sm text-[#a0a0b8] mb-4">Showcase and sell your artwork to thousands of art lovers.</p>
                ...
                >
                  ...
                  <h3 className="text-xl font-bold mb-2">Art Lover</h3>
                  <p className="text-sm text-[#a0a0b8] mb-4">Discover, buy, and collect artwork from independent artists.</p>
                  <ul className="text-xs text-[#6b6b80] space-y-2">
              ...
              <button
                onClick={() => setStep(2)}
                disabled={!role}
                className="w-full py-3 px-4 bg-white text-[#0a0a0f] hover:bg-gray-200 font-bold rounded-xl transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Continue
              </button>
            ...
            <div className="animate-in fade-in slide-in-from-right-8 duration-500 p-8 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-heading font-bold text-[#f0f0f5] mb-2">Account Details</h2>
                <p className="text-[#a0a0b8]">Complete your profile to get started</p>
              </div>
                ...
                {role === 'artist' && (
                  <>
                    <select value={artStyle} onChange={(e) => setArtStyle(e.target.value)} required
                      className="w-full bg-[#13131a] border border-white/10 rounded-xl py-3 px-4 text-[#f0f0f5] focus:outline-none focus:border-purple-500 appearance-none">
                    ...
                    <textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Short bio (optional)" rows={3}
                      className="w-full bg-[#13131a] border border-white/10 rounded-xl py-3 px-4 text-[#f0f0f5] focus:outline-none focus:border-purple-500 resize-none" />
                  </>
                )}

                <label className="flex items-center gap-3 cursor-pointer group mt-4">
                  <input type="checkbox" checked={agreed} onChange={(e) => setAgreed(e.target.checked)} className="w-4 h-4 rounded border-white/10 bg-[#13131a] text-purple-600 focus:ring-purple-500" />
                  <span className="text-sm text-[#a0a0b8] group-hover:text-[#f0f0f5]">I agree to the Terms of Service and Privacy Policy</span>
                </label>
