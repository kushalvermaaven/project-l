import React, { useState } from 'react';
import { Search, Star, Trash2, Eye } from 'lucide-react';
import DashboardLayout from '../../components/layout/DashboardLayout';
import Card from '../../components/ui/Card';
import Badge from '../../components/ui/Badge';
import Input from '../../components/ui/Input';
import Tabs from '../../components/ui/Tabs';
import { sampleArtworks } from '../../data/sampleData';
import { formatPrice } from '../../utils/helpers';

const ManageArtworks = () => {
  const [activeTab, setActiveTab] = useState('All');

  return (
    <DashboardLayout>
      <div className="mb-8">
        <h1 className="text-3xl font-heading font-bold text-white mb-2">Manage Artworks</h1>
      </div>

      <Card className="bg-white/5 border-white/10">
        <div className="p-6 border-b border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <Tabs tabs={['All', 'Published', 'Pending Review', 'Featured', 'Removed']} activeTab={activeTab} onChange={setActiveTab} />
          <div className="relative w-full md:w-64">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 w-5 h-5" />
            <Input placeholder="Search artworks..." className="pl-10" />
          </div>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-white/5 border-b border-white/10 text-xs uppercase tracking-wider text-gray-400">
                <th className="p-4 font-medium">Artwork</th>
                <th className="p-4 font-medium">Artist</th>
                <th className="p-4 font-medium">Price</th>
                <th className="p-4 font-medium">Status</th>
                <th className="p-4 font-medium">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {sampleArtworks.map((art, idx) => (
                <tr key={art.id} className="hover:bg-white/5 transition-colors">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <img src={art.images[0]} alt={art.title} className="w-12 h-12 rounded-lg object-cover" />
                      <div>
                        <p className="text-white font-medium">{art.title}</p>
                        <p className="text-xs text-gray-400">{art.category}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 text-gray-300">{art.artistId}</td>
                  <td className="p-4 text-white">{formatPrice(art.price)}</td>
                  <td className="p-4">
                    <Badge variant="success">Published</Badge>
                  </td>
                  <td className="p-4">
                    <div className="flex gap-2">
                      <button className="p-2 text-gray-400 hover:text-white bg-white/5 rounded-lg"><Eye className="w-4 h-4" /></button>
                      <button className={`p-2 bg-white/5 rounded-lg ${idx === 0 ? 'text-yellow-400' : 'text-gray-400 hover:text-yellow-400'}`}><Star className="w-4 h-4" fill={idx === 0 ? "currentColor" : "none"} /></button>
                      <button className="p-2 text-gray-400 hover:text-red-400 bg-white/5 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </Card>
    </DashboardLayout>
  );
};

export default ManageArtworks;
