...
router.get('/profile', verifyToken, async (req, res) => {
  const db = getDb();
  try {
    const user = await db.prepare('SELECT id, email, name, role, avatar, bio, location, phone FROM users WHERE id = ?').get(req.user.id);
    res.json(user);
  } catch (error) {
...
router.put('/profile', verifyToken, async (req, res) => {
  ...
  try {
    await db.prepare('UPDATE users SET name = ?, avatar = ?, bio = ?, location = ?, phone = ? WHERE id = ?')
